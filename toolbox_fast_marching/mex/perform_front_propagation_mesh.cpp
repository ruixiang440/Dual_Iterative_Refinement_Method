...
GW_Bool InsersionCallback( GW_GeodesicVertex& Vert, GW_Float rNewDist )
{
	...
	// display_message("nbr_iter=%d", nbr_iter);
	nbr_iter++;
	return doinsersion;
}
...
{ 
	...
	plhs[0] = mxCreateDoubleMatrix(nstart*nverts, 1, mxREAL);
	D = mxGetPr(plhs[0]);
	// second output : state
	plhs[1] = mxCreateDoubleMatrix(nstart*nverts, 1, mxREAL);
	S = mxGetPr(plhs[1]);
	// second output : segmentation
	plhs[2] = mxCreateDoubleMatrix(nstart*nverts, 1, mxREAL);
	Q = mxGetPr(plhs[2]);
...
//        if (i==0) continue;
        ...
        if (values != NULL)
            for (int j = i; j < i+1; ++j) {
                GW_GeodesicVertex *v = (GW_GeodesicVertex *) Mesh.GetVertex((GW_U32) start_points[j]);
                GW_ASSERT(v != NULL);
                v->SetDistance(values[j]);
            }
...
//	display_message("itermax=%d", niter_max);
        ...
        for (int j = 0; j < nverts; ++j) {
            GW_GeodesicVertex *v = (GW_GeodesicVertex *) Mesh.GetVertex((GW_U32) j);
            GW_ASSERT(v != NULL);
            D[j+nverts*i] = v->GetDistance();
            S[j+nverts*i] = v->GetState();
            GW_GeodesicVertex *v1 = v->GetFront();
            if (v1 == NULL)
                Q[j+nverts*i] = -1;
            else
                Q[j+nverts*i] = v1->GetID();
        }
//        i++;
//        for (int j = 0; j < nverts; ++j) {
//            GW_GeodesicVertex *v = (GW_GeodesicVertex *) Mesh.GetVertex((GW_U32) j);
